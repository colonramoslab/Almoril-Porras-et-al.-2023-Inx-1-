...
locPlot(track, sstart:send, AxesList(3), LocPlotOptions{:});
handles = [];
hloc = [];
...
for j = inds
    ...
    pt(j).drawTrackImage([],'Axes', AxesList(1), 'fid', fid, varargin{:}); 
...
function datafields(track, ax, inds, TitleOptions, DataPlotOptions, DataAxesOptions)
    ...
    mult = [rad2deg(1), 1, 1];
    yl = {[-120 120], [0 max(track.getDerivedQuantity(fields{2}))]};
    for k = 1:2
        hold(ax(k), 'off');
  ...
  %  set (ax(1), 'YAxisLocation', 'Right');
    if (false)
        ylabel (ax(1), '\leftarrow leftward ; rightward \rightarrow', TitleOptions{:});
        ylabel(ax(2), '\leftarrow slower ; faster \rightarrow', TitleOptions{:});
    end
end
