function plotPath2(tp, pathType, linetype, varargin)
...
if (length(tp) > 1)
    ih = ishold(Axes);
    for j = 1:length(tp)
        tp(j).plotPath2(pathType, linetype, 'Axes', Axes, varargin{:});
        hold (Axes,'on');
    end
    if (~ih)
        hold (Axes,'off');
...
tp.track.plotPath2(pathType, linetype, 'inds', si:ei, 'highlightinds', tp.startInd:tp.endInd, 'highlightlinetype', highlightlinetype, ...
    'Axes', Axes, varargin{:});
