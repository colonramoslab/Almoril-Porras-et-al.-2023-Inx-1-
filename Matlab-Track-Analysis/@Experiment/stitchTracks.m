...
%            map of tracks in and out
    ...
    for j = length(expt.track):-1:1
        ...
        firstValid = j + find(sf((j+1):lastValid) > expt.track(j).endFrame, 1, 'first'); %NACCHANGE switched greater to greater or equals
        ...
        for k = setdiff(firstValid:lastValid, alreadyMerged);
		    ...
		    break; % NACCHANGE
                ...
                continue; %only merge one track NACCHANGE
            end
