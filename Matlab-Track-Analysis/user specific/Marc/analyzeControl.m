...
basedir = 'D:\Marc Processed\maggots\control\';
...
if (restitchAndTrim)
    ...
    eset.executeExperimentFunction('pruneTracks', [], trimRect);
...
if (saveMe)
    ...
    tic
    save ([basedir 'importedToMatlab.mat'], 'control');
    toc
    saveMe = false;
end

fignum = 1;
eset.defaultTitle = 'Ethyl Acetate ~ 80 ppm at midpoint';


figure(fignum); clf(fignum); fignum = fignum+1; 
eset.makeHistogram('theta', deg2rad(-175.5:5:175.5), 'runs', 'r2d',true);
title ('distribution of instantaneous direction');

figure(fignum); clf(fignum); fignum = fignum+1; 

eset.gatherSubField('run', 'startTheta');
