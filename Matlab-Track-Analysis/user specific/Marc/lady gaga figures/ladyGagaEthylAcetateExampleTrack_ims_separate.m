...
hold off
ax1 = gca;
set(gca, 'XTick', [], 'YTick', [], 'LineWidth', 1, 'Box', 'on');
...
for j = 1:4
    ...
    track.pt(movinds(j)).drawTrackImage(track.expt.camcalinfo, 'fid', [], 'Axes', ax2(j), 'pretty', true, 'drawHeadArrow', false, 'spineColor', 'y.-', 'contourWidth', 1); 
    pt = track.expt.reloadPoint(track.pt(movinds(j)));
    insetim{insetimct} = uint8(255 - pt.imData);
    insetimct = insetimct + 1;
    hold(ax2(j), 'on');
...
for j = 1:4
    track.pt(movinds(j)).drawTrackImage(track.expt.camcalinfo, 'fid', [], 'Axes', ax(j), 'pretty', true, 'drawHeadArrow', false, 'spineColor', 'y.-', 'contourWidth', 1); 
    pt = track.expt.reloadPoint(track.pt(movinds(j)));
    insetim{insetimct} = uint8(255 - pt.imData);
    insetimct = insetimct + 1;
    hold(ax(j), 'on');
...
if (savefig)
    ...
    for k = 1:length(exts)
        fname = fullfile(SaveDirectory, ['example track ims separate', exts{k}]);
        saveas(gcf, fname, ftype{k});
    end
    warning(s);

    fname = fullfile(SaveDirectory, 'time lapse image.tiff');
    imwrite(uint8(255 - imtimelapse), fname, 'tiff');

    for j = 1:length(insetim)
        fname = fullfile(SaveDirectory, ['zoom image ' num2str(j) '.tiff']);
        imwrite(insetim{j}, fname, 'tiff');
    end
end
