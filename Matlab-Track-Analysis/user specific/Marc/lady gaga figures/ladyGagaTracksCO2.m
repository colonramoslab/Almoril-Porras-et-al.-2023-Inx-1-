if (~exist('eset', 'var'))
    eset = ExperimentSet.fromMatFiles('E:\larvalco2\Extracted\50 mL CO2 in 2 L air\CS\matfiles\cs_50', 'all');
    ecl = ESetCleaner;
    ecl.rpmCut = 1;
    ecl.askFirst = false;
    ecl.clean(eset);
...
t([t.startFrame] < 1000).plotPath('displacement','k-', 'Color', [0.4 0.4 0.4], 'LineWidth', 0.5); axis equal; hold on

%cc = 'rgbcmy';
cc = {[0.8 0 0], [0 0.8 0], [0 0 0.8], [0 0.6 0.6], [0.6 0 0.6], [0.5 0.5 0.1], [0.3 0.1 0.6]}; 
cind = 0;
for j = [189 115 202 65 149 75]
...
if (saveFig)
    SaveDirectory = 'C:\Users\Marc\Documents\figures\lady gaga paper\co2 spatial\cs co2 2500 ppm per cm';
    ...
    for k = 1:length(exts)
        fname = fullfile(SaveDirectory, ['co2 2500 ppm per cm tracks' exts{k}]);
