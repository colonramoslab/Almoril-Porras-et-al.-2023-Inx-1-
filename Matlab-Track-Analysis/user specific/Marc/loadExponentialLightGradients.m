...
ts1 = tic;
basedir = '\\LABNAS2\Phototaxis\Extracted Phototaxis Data\ReExtracted\Spatial Gradients\Exponential\';
esetname = 'lightExp';
if (fromScratch)
    close all;
      minpts = 50;
    ...
    if (~exist('lightExp', 'var'))
        lightExp = ExperimentSet.fromFiles(basedir, 'minpts', minpts);
    end
    ...
    lightExp.executeExperimentFunction('stitchTracks', frameDiff, maxDist, 'interactive', false);
    ...
    allloc = lightExp.gatherField('iloc');
    minl = min(allloc, [], 2);
    maxl = max(allloc, [], 2);
    
    buffer = 25;
    trimrect = [minl(1)+buffer minl(2)+buffer maxl(1)-buffer maxl(1)+buffer];
    lightExp.executeExperimentFunction('trimTracks', [], trimrect);
   ...
   % save (fullfile(basedir, 'marcmatfile.mat'), 'cryo');
    ...
    lightExp.toMatFiles(fullfile(basedir, 'matfiles',esetname));
    disp('saved file');
    toc(ts1)
    fromScratch = false;
else
    if (~exist('lightExp', 'var'))
        ...
        lightExp = ExperimentSet.fromMatFiles(fullfile(basedir, 'matfiles',esetname));
...
if (resegment)
    lightExp.executeTrackFunction('setSegmentSpeeds');
    lightExp.executeTrackFunction('segmentTrack');
    resegment = false;
end

sno.angleBinSize = 60;
sno.preferredDirection = 180;
%spatialNavigationMaggotFigures(lightExp, sno);
