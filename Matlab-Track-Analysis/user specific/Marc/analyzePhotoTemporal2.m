%basedir = '\\labnas1\Share\Ashley Extracted\temporal2\';
basedir = '\\labnas1\Share\Ashley Extracted\Temporal AllLight';
...
if (~exist('photo60', 'var'))
    minpts = 500;
    photo60 = ExperimentSet.fromFiles(basedir,'minpts',minpts);
end
...
badlimit = 0.94;
for j = 1:length(photo60.expt)
    br = photo60.expt(j).gatherField('ihtValid', 'mean');
    photo60.expt(j).track = photo60.expt(j).track(br > badlimit);
end
...
%%
if (~exist ('photo60old', 'var')) return; end;

fignum = fignum + 1; figure(fignum); clf(fignum);
[newlt,newlts] = photo60.meanField2vsField1('lighttime', 'speed', -60:2:60); 
[oldlt,oldlts] = photo60old.meanField2vsField1('lighttime', 'speed', -60:2:60); 
plot (newlt, newlts, oldlt, oldlts); 
legend ('new', 'old');
