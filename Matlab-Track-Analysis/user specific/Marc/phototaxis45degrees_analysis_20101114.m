...
ts1 = tic;
basedir = 'G:\maggot data\phototaxis\directional 45 degrees';
esetname = 'light45';
if (fromScratch)
    close all;
      minpts = 50;
    ...
    if (~exist('light45', 'var'))
        light45 = ExperimentSet.fromFiles(basedir, 'minpts', minpts);
    end
    ...
    % but if
    % you set interactive to true, it will show you each potential stitch
    % and
    % let you decide whether or not to stitch it
    light45.executeExperimentFunction('stitchTracks', frameDiff, maxDist, 'interactive', false);
    ...
    allloc = light45.gatherField('iloc');
    minl = min(allloc, [], 2);
    maxl = max(allloc, [], 2);
    
    buffer = 25;
    trimrect = [minl(1)+buffer minl(2)+buffer maxl(1)-buffer maxl(1)+buffer];
    light45.executeExperimentFunction('trimTracks', [], trimrect);
   ...
   % save (fullfile(basedir, 'marcmatfile.mat'), 'cryo');
    ...
    light45.toMatFiles(fullfile(basedir, 'matfiles',esetname));
    disp('saved file');
    toc(ts1)
    fromScratch = false;
else
    if (~exist('light45', 'var'))
        ...
        light45 = ExperimentSet.fromMatFiles(fullfile(basedir, 'matfiles',esetname));
...
if (resegment)
    light45.executeTrackFunction('setSegmentSpeeds');
    light45.executeTrackFunction('segmentTrack');
    resegment = false;
end

sno.angleBinSize = 60;
sno.preferredDirection = 180;
ad = spatialNavigationMaggotAnalysis(light45, sno);
spatialNavigationMaggotFigures(light45, sno);
