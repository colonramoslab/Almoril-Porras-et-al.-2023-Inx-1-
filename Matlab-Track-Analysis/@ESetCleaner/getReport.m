function num2clean = getReport(ecl, eset)
...
disp(msg);
msg = [num2str(sum(npts(npts < ecl.minPts))) '/' num2str(sum(npts)) ' pts removed by displacement test (' num2str(sum(npts(npts < ecl.minPts))/sum(npts)*100,2), ' %)'];
disp(msg);
