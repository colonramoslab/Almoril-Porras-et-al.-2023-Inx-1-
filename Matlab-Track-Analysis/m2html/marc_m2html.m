	...
	switch(optionsnames{ind})
		case 'verbose'
			...
			elseif strcmpi(pvalue,'off')
				...
				error(sprintf(msgInvalidPair,pname)); %#ok<*SPERR>
			end
		case 'mfiles'
			if iscellstr(pvalue)
		...
		case 'load'
			if ischar(pvalue)
                if exist(pvalue, 'dir') == 7 % directory provided
...
%-------------------------------------------------------------------------------
for i=1:length(mdir)
	if ~exist(fullfile(options.htmlDir,mdir{i}), 'dir') 
		ldir = splitpath(mdir{i});
		for j=1:length(ldir)
			if ~exist(fullfile(options.htmlDir,ldir{1:j}), 'dir')
				...
				if j == 1
                    %escapeblank(ldir{1})
                    [~, msg] = mkdir((options.htmlDir), ...
						(ldir{1}));
					%[~, msg] = mkdir(escapeblank(options.htmlDir), ...
					%	escapeblank(ldir{1}));
                    %msg
                else
