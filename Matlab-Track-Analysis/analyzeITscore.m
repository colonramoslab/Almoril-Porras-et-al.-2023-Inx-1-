function [fPath, hRat, ITscore] = analyzeITscore(fPath, varargin)
...
smW=50; % window for smoothing to find speed.50
...
h1v=histcounts(xDisp(isIT)-minX,hMin:(hMax-hMin)/20:hMax);
h2v=histcounts(xDisp(notIT)-minX,hMin:(hMax-hMin)/20:hMax);
hRat=h1v./(h1v+h2v);
...
saveName=[prefix,'_ITtrackscore'];
save(fullfile(saveDir,saveName),'ITtrackscore');
