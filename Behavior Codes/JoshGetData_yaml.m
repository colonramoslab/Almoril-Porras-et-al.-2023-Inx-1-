function [trackMatrix3D,trackHeaders,trackFileList] = JoshGetData_yaml(eset)
 
% Reports track properties for all tracks from experiments in eset
% as a 3D matrix in format [track, property, expt]
% properties are listed as trackHeaders array
% source files are listed in trackFileList
...
trackHeaders = {'Track','Sharp Turns','Omega Turns','Blip Turns','Reversals','Second Reversals','Xdisp','Average Run Length','Positive Average Run Length','Positive Average X Displacement','Negative Average Run Length','Negative Average X Displacement','Positive Run Count','Negative Run Count','Fraction Neg Runs','Negative Bias Count','Positive Bias Count','posTimeAvg','negTimeAvg','posVelocityAvg','negVelocityAvg','posCurveIndexAvg','negCurveIndexAvg','posWVIAvg','negWVIAvg','StartCountBias','StartLengthBias','TTXI','TTXV','absTTXV','drift','Obias','Lbias', 'Tbias', 'Cbias','Vbias','WVbias'};

% 'Obias','Lbias', 'Tbias', 'Cbias','Vbias','WVbias'


propertyLength = size(trackHeaders,2);
propertySize = [1:propertyLength];

trackFileList =  {}; %Store list of files for each track in cell string array
            %columns are tracks within same experiment, rows are new expt
trackMatrix3D = zeros(24,31,0); %will store across expts in z-stack
...
for i = 1:length(eset)
    for j = 1:length(eset(i).expt);
       ...
       % fprintf(textfile,'Expt,Track,# Sharp Turns,Omega Turns,Blip Turns,Reversals,Second Reversals,Distance Traveled,Average Run Length,Positive Average Run Length,Positive Average X Displacement,Negative Average Run Length,Negative Average X Displacement,Positive Run Count,Negative Run Count,Fraction Neg Runs,Negative Bias Count,Positive Bias Count,posTimeAvg,negTimeAvg,posVelocityAvg,negVelocityAvg,posCurveIndexAvg,negCurveIndexAvg,posWVIAvg,negWVIAvg,StartCountBias,StartLengthBias,TTXI,TTXV,absTTXV,drift\n');

       %Initialize for new dataset
        ...
        for k = 1:length(eset(i).expt(j).track);
            ...
            Obias = FracNeg;
            Lbias = negRunAvg/(negRunAvg+posRunAvg);
            Tbias = negTimeAvg/(negTimeAvg+posTimeAvg);
            Cbias = negCurveIndexAvg/(negCurveIndexAvg+posCurveIndexAvg);
            Vbias = negVelocityAvg/(negVelocityAvg+posVelocityAvg);
            WVbias = negWVIAvg/(negWVIAvg + posWVIAvg);
            ...
            %Drift velocity per worm, in frames... sec conversion is 0.4960
            framesToSec = 0.4960;
            trackTime = framesToSec*double(eset(i).expt(j).track(1,k).endFrame - eset(i).expt(j).track(1,k).startFrame);
            drift = double(XDisp)/trackTime;
            

            
            % ADD DATA INTO THE matrix
            trackMatrix3D(k,propertySize,j) = [k,numSharpTurns,turnOmega,turnBlip,turnReversal,turnSecondRev,double(XDisp),averageRunLength,posRunAvg,posXDispAvg,negRunAvg,negXDispAvg,posRuns,negRuns,FracNeg,negBiasCount,posBiasCount,posTimeAvg, negTimeAvg, posVelocityAvg, negVelocityAvg, posCurveIndexAvg, negCurveIndexAvg, posWVIAvg, negWVIAvg, StartCountBias, StartLengthBias, ttxi,double(ttxv),double(abs(ttxv)),drift,Obias,Lbias,Tbias,Cbias,Vbias,WVbias];  
            trackFileList{k,j} =  eset(i).expt(j).fname;                   
