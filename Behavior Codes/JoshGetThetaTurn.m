...
for i = 1:length(eset)
    for j = 1:length(eset(i).expt);
        ...
        fprintf(turnfile,'Expt,Track,Turn#,Omega,Blip,Reversal,2ndReversal\n');
        ...
        for k = 1:length(eset(i).expt(j).track);
            ...
            for m = 1:length(eset(i).expt(j).track(1,k).sharpTurn)
                if eset(i).expt(j).track(1,k).sharpTurn(1,m).typeCode == -1
                    ...
                    TotalOmega = TotalOmega + 1;
                elseif eset(i).expt(j).track(1,k).sharpTurn(1,m).typeCode == 0
                    turnBlip = turnBlip + 1;
                    TotalBlip = TotalBlip + 1;
                elseif eset(i).expt(j).track(1,k).sharpTurn(1,m).typeCode == 1
                    turnReversal = turnReversal + 1;
                    TotalReversal = TotalReversal + 1;
                elseif eset(i).expt(j).track(1,k).sharpTurn(1,m).typeCode == 2
                    turnSecondRev = turnSecondRev +1;
                    TotalSecondRev = TotalSecondRev + 1;
                end
                
                % print foturn data for each run
                fprintf(turnfile,'%s,%i,%i,%i,%i,%i,%i\n',eset(i).expt(j).fname,k,m,turnOmega,turnBlip,turnReversal,turnSecondRev);
            ...
            % NOTE: Matt's turn output is not quite right.... Fix the carry
            % or write over issue
            fprintf(turnfile,'\nExpt,Track,TotalOmega,TotalBlip,TotalReversal,Total2ndReversal\n');
            fprintf(turnfile,'%i,%i,%i,%i,%i\n',k,TotalOmega,TotalBlip,TotalReversal,TotalSecondRev);
