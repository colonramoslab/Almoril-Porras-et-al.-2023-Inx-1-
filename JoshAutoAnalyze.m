...
while ischar(tline) %loop until eof
    if(strncmp(tline,'    output file:',16))   %look for "output file" line
        ...
        saveas(fignum,strcat(outputdirectory,fileprefix,'_initial.jpg'));
        close(fignum);
        ...
        if length(numpts) > 23 %23
            ptscutoff = numpts(24); %24
        else
            ptscutoff = 0;
        end
        ...
        nathanPlotTracks
        set(gcf, 'Position', get(0,'Screensize'));
        saveas(fignum,strcat(outputdirectory,fileprefix,'_final.jpg'));
        close(fignum);
        
        %plot superimposed tracks and save image
        nathanPlotSuperimposed
        set(gcf, 'Position', get(0,'Screensize'));
        saveas(fignum,strcat(outputdirectory,fileprefix,'_super.jpg'));
        close(fignum);
