function [] = JoshAutoAnalyze(filename, varargin)
...
while ischar(tline) %loop until eof
    if(strncmp(tline,'    output file:',16))   %look for "output file" line
        ...
        ecl=ESetCleaner();
        ecl.askFirst=false;
        ecl.minSpeed=0.25; % formerly 0.1
        ...
        if fullOpt
            ...
            %get rundata
            %[textfile, msg] = fopen(strcat(outputdirectory,fileprefix,'_runs_data.csv'),'w');
            %JoshGetData_WVI_Gusmodruns
            %fclose(textfile);
