function [fPath, hRat, ITscore] = visualizeITtracksMalcomEdit(fPath, varargin)
...
contThresh=45; % super arbitrary atm
prefix='';
...
itDiff=diff(smt1);
orthoDiff=diff(smt2);
speed=sqrt(itDiff.^2+orthoDiff.^2); %in units of pix/(0.5s)
fracITspeed=abs(itDiff)./speed;
avgTheta=atan2d(itDiff,orthoDiff);
avgdeltatheta = abs(diff(avgTheta));
fillerRow = ones(1, size(avgdeltatheta, 2)); %used to match passVals dimensions
avgdltThetawFiller = [ fillerRow; avgdeltatheta];
colavgdltThetawFiller = avgdltThetawFiller(:);
deltathetaThresh = 25; %angle differences higher than this are not runs, used to mark turns/reorientations
...
speed=speed(inVals);
colavgdltThetawFiller=colavgdltThetawFiller(inVals);
xDisp=xDisp(inVals);
yDisp=yDisp(inVals);
...
speedThresh=0.00;
yspeedThresh=0.0;
contThresh=45; % Could instead filter by physical length of continuity. 100
...
[~, runSegmentLengths] = stretchCounter(isIT);
runDurations = runSegmentLengths*0.5;
DurFname = strcat(char(prefix),'_RunDurs_eachRun_AllTracks.csv');
writematrix(runDurations, fullfile(saveDir,DurFname));
...
saveName=strcat(char(prefix),'_ITtrackscore.csv');
writematrix(ITtrackscore,fullfile(saveDir,saveName));
